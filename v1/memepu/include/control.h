...
class ControlLogic {
 ...
 private:
  ...
  void put(uint32_t addr, uint32_t output) {
    verify((addr & 0x03FFFF) == addr, "only 18 bits for address");  // 19th bit not currently used.
  ...
  void write(
      Opcode opcode0, int aux, int int_flag, int mmu_flag, int microop_idx, uint32_t microop_data);

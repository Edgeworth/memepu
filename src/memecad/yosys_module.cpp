...
std::vector<Schematic::SchematicFile> convertVerilogToKicadSchematics(
    const std::string& memecad_map_filename, std::vector<string> verilog_filenames,
  ...
  for (const auto& filename : verilog_filenames) yosys_cmd += " " + filename;
...
void TestPass::execute(std::vector<std::string> args, Design* design) {
  ...
  for (auto& module_iter : design->modules_) {
    ...
    if (!mapper_.isMappedModule(*module)) {
      ...
      for (auto& cell_iter : module->cells_) {
        ...
        verify(child_module != nullptr, "expected module %s' to exist", moduleType(*cell).c_str());
  ...
  for (auto& kv : design->modules_) {
    if (!mapper_.isMappedModule(*kv.second) && incoming[kv.second] == 0) next.push_back(kv.second);
  }
...
void TestPass::printModuleInfo(Yosys::RTLIL::Module* module) {
  ...
  for (auto& cell_iter : module->cells_) {
    const auto& [cell_id, cell] = cell_iter;
    log("  Cell %s, name: %s, module: %s\n    type: %s\n", cell_id.c_str(), cell->name.c_str(),
        cell->module->name.c_str(), cell->type.c_str());
  ...
  for (auto& sigsig : module->connections()) {
    log("   %s (size: %d) <=> %s (size: %d)\n", log_signal(sigsig.first), sigsig.first.size(),
        log_signal(sigsig.second), sigsig.second.size());
  }
  log("\n");
}

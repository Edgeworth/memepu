...
void Compile::extractSymbols(const Parser::Node* node) {
  switch (node->type) {
    ...
    default:
      compile_error(false, node, "unexpected construct at %d:%d: %s",
          contents_->getLineNumber(node->loc),
...
FuncSig Compile::extractFunctionSignature(const Parser::Node* node) {
  return FuncSig{
      ...
      map(findAll(node, Parser::Node::VARIABLE_DECLARATION),
          &Compile::extractVariableDeclaration), // params
...
const Parser::Node* Compile::maybeFindOne(const Parser::Node* node, Parser::Node::Type type) {
  ...
  compile_error(nodes.size() <= 1, node, "expected no more than one node of type %d, found %d",
      int(type),
      int(nodes.size()));
  return nodes[0];
}
...
const Parser::Node* Compile::findOne(const Parser::Node* node, Parser::Node::Type type) {
  auto nodes = findAll(node, type);
  compile_error(nodes.size() == 1, node, "expected only one node of type %d, found %d", int(type),
      int(nodes.size()));
  return nodes[0];
}

std::vector<const Parser::Node*>
Compile::findAll(const Parser::Node* node, Parser::Node::Type type) {

...
Point transformPoint(const Point& p, Side side, const Point& translation, int rotation) {
  Point world = p;
  if (side == Side::BACK) world = -world;
  switch (rotation) {
  case 0: break;
  case 90: world = {-world.y, world.x}; break;
  case 180: world = {-world.x, -world.y}; break;
  case 270: world = {world.y, -world.x}; break;
  default: verify_expr(false, "only support axis aligned rotations, not '%d'", rotation);
  }
...
Shape transformShape(Shape s, Side side, const Point& translation, int rotation) {
  ...
  case Shape::Type::PATH:
    for (auto& p : s.path.points) p = transformPoint(p, side, translation, rotation);
    break;
...
void Pcb::verifyAndSetup() {
  ...
  for (const auto& kv : components) {
    verify_expr(
        images.count(kv.second.image_id) > 0, "missing image '%s'", kv.second.image_id.c_str());
  }
  ...
  for (const auto& kv : padstacks)
    for (const auto& shape : kv.second.shapes)
      verify_expr(shape.layer_id != -1,
          "padstack shapes must have specified layer id since they aren't associated with a "
          "component");
